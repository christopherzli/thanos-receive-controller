...
func (c *controller) sync(ctx context.Context) {
	...
	for _, obj := range c.ssetInf.GetStore().List() {
		level.Info(c.logger).Log("msg", "informer sts list", "list", fmt.Sprintf("%+v", c.ssetInf.GetStore().List()))
	...
	level.Info(c.logger).Log("msg", "start to populate hashring", "statefulsets", fmt.Sprintf("%+v", statefulsets))
